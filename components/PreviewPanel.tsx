...
/* ── Mockup card shown when the iframe renders blank (SPAs, JS-heavy sites) ── */
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-start gap-2 rounded-xl px-4 py-3 text-xs"
        ...
        <span>This site uses JavaScript rendering so the live iframe shows blank.
          Showing a <strong>before / after mockup</strong> of the copy changes instead.</span>
      </div>
...
export default function PreviewPanel({ modifiedHtml, step, changes, originalPage, adInsights }: Props) {
  ...
  function handleIframeLoad() {
    try {
      ...
      // If rendered body has < 120 chars of text it's a blank/SPA page
      if (text.length < 120) setShowMock(true);
    } catch {
      setShowMock(true);
    }
  }
  ...
  /* ── Done — mock view ── */
  if (showMock && changes && originalPage && adInsights) {
    return (
